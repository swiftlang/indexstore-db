...
namespace sys {
  ...
  public:
    ...
    static llvm::sys::TimeValue fromTimePoint(llvm::sys::TimePoint<> tp) {
      llvm::sys::TimePoint<std::chrono::seconds> truncated = std::chrono::time_point_cast<std::chrono::seconds>(tp);
      auto seconds = std::chrono::duration_cast<std::chrono::seconds>(truncated.time_since_epoch()).count();
      auto nanoseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(tp - truncated).count();
      TimeValue result;
      result.fromEpochTime(seconds);
      result.nanos_ = nanoseconds;
      return result;
    }
